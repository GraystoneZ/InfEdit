...
class SSIM(torch.nn.Module):
    ...
    def __init__(self, channels=3, downsample=False, test_y_channel=True, color_space='yiq', crop_border=0., get_ssim_map=False):
    ...
    def forward(self, X, Y):
        ...
        score = ssim(X, Y, data_range=self.data_range, downsample=self.downsample, get_ssim_map=self.get_ssim_map)
        return score
...
def ssim_structure(X,
    ...
    # cs_map = F.relu(cs_map)  # force the ssim response to be nonnegative to avoid negative results.
    # ssim_map = ((2 * mu1_mu2 + C1) / (mu1_sq + mu2_sq + C1)) * cs_map
    ssim_map = (sigma12 + C3) / (sigma1 * sigma2 + C3)
    ssim_map = F.relu(ssim_map)
    ssim_val = ssim_map.mean([1, 2, 3])
...
@ARCH_REGISTRY.register()
class SSIM_structure(torch.nn.Module):
    r"""Args:
        channel: number of channel.
        downsample: boolean, whether to downsample same as official matlab code.
        test_y_channel: boolean, whether to use y channel on ycbcr same as official matlab code.
    ...
    def __init__(self, channels=3, downsample=False, test_y_channel=True, color_space='yiq', crop_border=0., get_ssim_map=False):
        ...
        super(SSIM_structure, self).__init__()
        self.downsample = downsample
        self.test_y_channel = test_y_channel
        self.color_space = color_space
        self.crop_border = crop_border
